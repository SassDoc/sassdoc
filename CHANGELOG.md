...
* `--sass-convert` option does not exist anymore, it is now implied ([]())
...
* `$` sign is now optional when defining the name in `@parameter` annotation
...
* `@content` annotation is now correctly output in default theme
